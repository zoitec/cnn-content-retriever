...
class ContentRetriever {
        ...
        return new Promise((resolve) => {
            ...
            request.get({json: true, url: url, timeout: self.timeout}, (error, response, body) => {
                if (error) {
                    ...
                    let urlKey = sourceModel.docs[0].url
                                    .replace(/http(s?):\/\/(www|edition).cnn.com/, '')
                                    .replace(/\./g, '_')
                                    .replace(/\-/g, '_')
                                    .replace(/\//g, '_');

                    if (sourceModel.docs[0].coverArt) {
                        sourceModel.docs[0].coverArt = body[urlKey].coverArt;
                    }
     ...
     * );
     */
    getRecentPublishes(limit, contentTypes, dataSources) {
        if (limit && limit > 100) {
            limit = 100;
        }
        ...
        const self = this,
            ...
            url = `${self.hypatiaHost}${self.hypatiaRoute}${typeQuery}${dataSourceQuery}${limitQuery}sort:lastPublishDate`;
